...
const CheckpointPage = () => {
  ...
  const { state, addCheckpoint } = useAppData();
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Не удалось сохранить чекпоинт');
    }
  };
    ...
    return (
      <GlassCard>
        <div className="space-y-3 text-sm text-muted">
          <p>Для сохранения чекпоинта войдите в профиль.</p>
